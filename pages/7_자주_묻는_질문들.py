...
# FAQ 항목 1
with st.expander(expanded=True,label=":red[Q. 뇌졸중은 호전될 수 있는 병인가요? (팔다리를 다시 움직일 수 있을까요?)]"):
    st.write("""
...
# FAQ 항목 2
with st.expander(expanded=True,label=":red[Q.치료에는 시간이 어느 정도 걸리나요?]"):
    st.write("""
...
# FAQ 항목 3
with st.expander(expanded=True,label=":red[Q.뇌졸중이 재발할 수 있나요?]"):
    st.write("""
...
# FAQ 항목 4
with st.expander(expanded=True,label=":red[Q.뇌졸중 발생 시 할 수 있는 응급조치는 무엇인가요?]"):
    st.write("""
...
# FAQ 항목 5
with st.expander(expanded=True,label=":red[Q.뇌졸중 후 운전할 수 있나요?]"):
    st.write("""
...
# FAQ 항목 6
with st.expander(expanded=True,label=":red[Q.뇌졸중 환자는 고기를 먹으면 안 되나요?]"):
    st.write("""
...
# FAQ 항목 7
with st.expander(expanded=True,label=":red[Q.짜게 먹으면 뇌졸중에 잘 걸리나요?]"):
    st.write("""
...
# FAQ 항목 8
with st.expander(expanded=True,label=":red[Q.뇌졸중 환자가 커피를 마셔도 되나요?]"):
    st.write("""
